...
>
> **窗口前提（默认双边界、两端同值）**：本模板与其派生插件的窗口是
> `[<目标版本>, <目标版本>]`——peer/dev 写同一个 `^<目标版本>`，门禁用
> `[MIN_HARNESS_VERSION, MAX_HARNESS_VERSION]` 双边界比较（当前两端同值），
> 代码里**不留版本探测分支**。如确需多线支持，参考
...
窗口保持双边界：两端同值时即"仅支持该版本"，需要支持新版本时只上调 MAX——
不要随意加宽到未实测的 tag（跨版本 API 破坏频繁，范围放宽救不了）。
