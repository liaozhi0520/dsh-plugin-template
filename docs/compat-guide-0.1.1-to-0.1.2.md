...
> ⚠️ **历史归档（2026-09-12）**：本手册服务于双窗口 `[0.1.1-rc.2, 0.1.2-rc.1]`。
> 当前窗口已收敛为**单窗口 `[0.1.5-rc.1, 0.1.5-rc.1]`**——现行手册是
> `docs/compat-guide-0.1.2-to-0.1.5.md`，升级方案见派生插件任一份
> `docs/dsh-0.1.5-rc.1-upgrade-report.md`。本文仅在确需重新引入双线支持、
> 或排查跨 0.1.1/0.1.2 线行为差异时查阅；其"peer 写并集 + dev 钉死一条线"
> 的依赖写法在单窗口下**不适用**。
